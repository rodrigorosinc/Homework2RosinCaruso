...
class Time {
    ...
    void changeTime(int h, int m, int s, std::string period) {
        if (!isValidHour(h)) {throw std::invalid_argument("Invalid hour");}
        if (!isValidMinute(m)) {throw std::invalid_argument("Invalid minute");}
        if (!isValidSecond(s)) {throw std::invalid_argument("Invalid second");}
        if (!isValidPeriod(period)) {throw std::invalid_argument("Invalid period");}
...
int main (){
    ...
    while(error){
        try {
            ...
            std::cout << "1. Constructor with hour" << std::endl;
            std::cout << "2. Constructor with hour and minute" << std::endl;
            std::cout << "3. Constructor with hour, minute and second" << std::endl;
            ...
            if (constructor == default_constructor) {
                ...
                std::cout << "Enter hour and minute: ";
                std::cin >> h >> m;
                Time t3(h, m);
                t3.readTime();
                ...
                std::cout << "Enter hour, minute, second and period: ";
                std::cin >> h >> m >> s >> period;
